...
int status_change_end(struct block_list* bl, enum sc_type type, int tid)
{
	...
	if (type == SC_AUTOATTACK && sc->getSCE(type)->val4 > 0)
		pc_setreg(sd, add_str("@AA_LTICK"), sc->getSCE(type)->val4 * 100); //Record how many time you left
...
TIMER_FUNC(status_change_timer){
		...
		if (--(sce->val4) > 0) {
					...
					if(!sd->aa.autoheal.empty() && hit_ > 2000){
						if(last_tick >= sd->aa.skill_cd){
							for(auto &itAutoheal : sd->aa.autoheal){
								if(((status->hp * 100 / itAutoheal.min_hp) < sd->status.max_hp) 
									&& pc_checkskill(sd, itAutoheal.skill_id) >= itAutoheal.skill_lv){
										...
										if(last_tick >= itAutoheal.last_use){
											if(unit_skilluse_id(bl, bl->id, itAutoheal.skill_id, itAutoheal.skill_lv)){
												itAutoheal.last_use = last_tick + + skill_get_cooldown( itAutoheal.skill_id, itAutoheal.skill_lv );
												skip = true;
			...
			{//Buff skills
				if(battle_config.feature_autoattack_buffskill){
					if(!skip && !pc_issit(sd) && !sd->aa.autobuffskills.empty()){
						if(last_tick >= sd->aa.skill_cd){
							for(auto &itAutobuffskills : sd->aa.autobuffskills){
								if(last_tick >= sd->aa.skill_cd && last_tick >= itAutobuffskills.last_use){
									if(itAutobuffskills.is_active && !skill_isNotOk(itAutobuffskills.skill_id, sd) && pc_checkskill(sd, itAutobuffskills.skill_id) >= itAutobuffskills.skill_lv && !sc->getSCE(skill_get_sc(itAutobuffskills.skill_id))){
										if(unit_skilluse_id(&sd->bl, sd->bl.id, itAutobuffskills.skill_id, itAutobuffskills.skill_lv)){
											itAutobuffskills.last_use = last_tick + skill_get_cooldown( itAutobuffskills.skill_id, itAutobuffskills.skill_lv );
											skip = true;

											if(itAutobuffskills.last_use > sd->aa.skill_cd)
												sd->aa.skill_cd = itAutobuffskills.last_use;
											if(sd->aa.skill_cd < (last_tick + skill_get_delay( itAutobuffskills.skill_id, itAutobuffskills.skill_lv )) )
												sd->aa.skill_cd = last_tick + skill_get_delay( itAutobuffskills.skill_id, itAutobuffskills.skill_lv );
										}
									}
								} else
									skip = true;
			...
			{//Attack skills
				//ShowError("1 - Attack Skill skip %d sd->aa.target_id %d sd->aa.attack_target_id %d sd->aa.itempick_id %d \n", skip,sd->aa.target_id,sd->aa.attack_target_id,sd->aa.itempick_id);
						...
						if(sd->aa.autoattackskills.size() > 0){
							if(last_tick >= sd->aa.skill_cd){
								for(auto &itAutoattackskills : sd->aa.autoattackskills){
									...
									if(last_tick >= itAutoattackskills.last_use && random_skill <= 100){ // 25% luck of using skill
										if(itAutoattackskills.is_active && !skill_isNotOk(itAutoattackskills.skill_id, sd) && pc_checkskill(sd, itAutoattackskills.skill_id) >= itAutoattackskills.skill_lv){
											unit_stop_attack(bl);
										...
										if(sd->aa.skill_cd < (last_tick + skill_get_delay( itAutoattackskills.skill_id, itAutoattackskills.skill_lv )) )
											sd->aa.skill_cd = last_tick + skill_get_delay( itAutoattackskills.skill_id, itAutoattackskills.skill_lv );
										if(sd->aa.skill_cd < last_tick + ((1*battle_config.feature_autoattack_skilldelay)*1000))
											sd->aa.skill_cd = last_tick + ((1*battle_config.feature_autoattack_skilldelay)*1000);
									}
								}
							}
						}
			...
			{//Move
				...
				if(!skip && !pc_issit(sd) && ((sd->aa.target_id == 0 && sd->aa.itempick_id == 0) || attack_ > 10000) && !flywing_){
					const int d=battle_config.feature_autoattack_move; // number of cell a player can move
	...
	case SC_NPC_KILLING_AURA:
		if (--(sce->val4) >= 0) {
			skill_castend_nodamage_id(bl, bl, NPC_KILLING_AURA, sce->val1, tick, 1);
			sc_timer_next(1000 + tick);
			return 0;
		}
		break;
