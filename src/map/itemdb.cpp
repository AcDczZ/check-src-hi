...
void ItemDatabase::loadingFinished(){
	for (auto &tmp_item : item_db) {
		...
		// Custom Extreme
		if (battle_config.sell_piece_cap != 0 && item->value_sell > battle_config.sell_piece_cap && item->type != IT_CARD) {
            item->value_sell = battle_config.sell_piece_cap;
        }
		if (battle_config.sell_piece_cap_card != 0 && item->type == IT_CARD && item->subtype == CARD_NORMAL) {
            item->value_sell = battle_config.sell_piece_cap_card;
        }
		
		if ( item->type == IT_ARMOR || item->type == IT_WEAPON ){
			item->value_sell = item->value_buy / 20;
		}
