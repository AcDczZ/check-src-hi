...
void read_int_array_from_file(const char* filename, long** array, long* size) {
	...
	char line[3000];
	int capacity = 20;
	*array = (long*)malloc(capacity * sizeof(long));
	*size = 0;
	...
	while (fgets(line, sizeof(line), file)) {
		...
		// Split line into tokens and parse integers
		char* token = strtok(line, " \t\n");
		while (token != NULL) {
			if (*size >= capacity) {
				capacity *= 2;
				*array = (long*)realloc(*array, capacity * sizeof(long));
			}
...
static int storage_additem(struct map_session_data* sd, struct s_storage *stor, struct item *it, int amount)
{
	...
	read_int_array_from_file("db/collection.txt", &collection, &collection_size);

	// Iterate over the collection array and print each item ID
		...
		for (i = 0; i < stor->max_amount; i++) {
			if (compare_item(&stor->u.items_storage[i], it)) { // existing items found, stack them
				...
				if (sd->state.storage_flag == 4)
					{
					if (stor->u.items_storage[i].amount >= 1 || data->stack.storage && amount >= 1) { // item stack limitation
						clif_messagecolor(&sd->bl, color_table[COLOR_RED], "This Item has in Collection", false, SELF);
						return 1;
	...
	if (sd->state.storage_flag == 4)
		{
		//int collection[] = { 2220,2222,60059,2224,2225,2230 };
		for (i = 0; i < data->nameid; i++) {
			
				ARR_FIND(0, i, listidx, collection[listidx] == data->nameid);
			
				if (collection[listidx] == 0) {
				clif_messagecolor(&sd->bl, color_table[COLOR_RED], "This Item is not Collection", false, SELF);
				return 1;
			...
			for (i = 0; i < stor->max_amount; i++) {
					...
					if (i) {
					ARR_FIND(0, i, listidx, stor->u.items_storage[listidx].nameid == it->nameid);
					
												// duplicate
						if (listidx != i) {
													//ShowWarning("buyingstore_create: Found duplicate item on buying list (nameid=%u, amount=%hu).\n", it->nameid, it->amount);
							clif_messagecolor(&sd->bl, color_table[COLOR_RED], "This Item has in Collection", false, SELF);
						return 2;
		...
		else
			{
						// find free slot
				ARR_FIND(0, stor->max_amount, i, stor->u.items_storage[i].nameid == 0);
						//ShowWarning("OK find item TEST: %d\n", stor->u.items_storage[i].nameid);
				if (i >= stor->max_amount)
				return 2;
			}
...
int storage_delitem(map_session_data* sd, struct s_storage *stor, int index, int amount)
{
	...
	if (stor->u.items_storage[index].amount == 0) {
		...
		stor->amount--;
		if (sd->state.storage_flag == 1 || sd->state.storage_flag == 3 || sd->state.storage_flag == 4)
			clif_updatestorageamount(sd, stor->amount, stor->max_amount);
	}
 ...
 * @param sd Player
 **/
void storage_premiumStorage_open2(map_session_data* sd) {
	nullpo_retv(sd);
...
/**
 * Request to open Collection storage
 * @param sd Player who request
 * @param num Storage number
 * @param mode Storage mode @see enum e_storage_mode
 * @return 1:Success to request, 0:Failed
 **/
bool storage_premiumStorage_load2(map_session_data* sd, uint8 num, uint8 mode) {
	...
	if (sd->CollectionStorage.stor_id != num)
		...
		storage_premiumStorage_open2(sd);
	}
...
void storage_premiumStorage_close(map_session_data *sd) {
	...
	if (sd->premiumStorage.dirty || sd->CollectionStorage.dirty) {
		if (save_settings&CHARSAVE_STORAGE)
			chrif_save(sd, CSAVE_INVENTORY|CSAVE_CART);
		else
	...
	if (sd->state.storage_flag == 3 || sd->state.storage_flag == 4) {
		sd->state.storage_flag = 0;
		clif_storageclose( sd );
	}
}
