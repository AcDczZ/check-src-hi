...
bool pc_authok(map_session_data *sd, uint32 login_id2, time_t expiration_time, int group_id, struct mmo_charstatus *st, bool changing_mapservers)
{
	...
	memset(&sd->CollectionStorage, 0, sizeof(struct s_storage));
...
void pc_aa_load(map_session_data* sd){
	...
	if (Sql_Query(mmysql_handle,
		"SELECT `stopmelee`,`pickup_item_config`,`aggressive_behavior`,`autositregen_conf`,`autositregen_maxhp`,`autositregen_minhp`,`autositregen_maxsp`,`autositregen_minsp`,`tp_use_teleport`,`tp_use_flywing`,`tp_min_hp`,`tp_delay_nomobmeet` "
...
void pc_bonus3(map_session_data *sd,int type,int type2,int type3,int val)
{
	...
	case SP_RAMDOMOPTIONCOUNT: // bRandomoptioncount,val;
		break;
...
static void pc_calcexp(map_session_data *sd, t_exp *base_exp, t_exp *job_exp, struct block_list *src)
{
	...
	if( sd->sc.getSCE(SC_VOTELV1) )
		bonus += 2;		
	if( sd->sc.getSCE(SC_VOTELV2) )
		bonus += 3;		
	if( sd->sc.getSCE(SC_VOTELV3) )
		bonus += 4;		
	if( sd->sc.getSCE(SC_VOTELV4) )
		bonus += 10;		
	if( sd->sc.getSCE(SC_VOTELV5) )
		bonus += 12;		
	if( sd->sc.getSCE(SC_VOTELV6) )
		bonus += 15;		
	if( sd->sc.getSCE(SC_VOTELV7) )
		bonus += 17;		
	if( sd->sc.getSCE(SC_VOTELV8) )
		bonus += 18;		
	if( sd->sc.getSCE(SC_VOTELV9) )
		bonus += 20;		
	if( sd->sc.getSCE(SC_VOTELV10) )
		bonus += 25;
	
	// APACHE PREMIUM SERVICE
	if (sd->sc.getSCE(SC_APACHE_EXPBOOST_A)) {
		bonus += sd->sc.getSCE(SC_APACHE_EXPBOOST_A)->val1;
		if (battle_config.vip_bm_increase && pc_isvip(sd)) // Increase Battle Manual EXP rate for VIP
			bonus += (sd->sc.getSCE(SC_APACHE_EXPBOOST_A)->val1 / battle_config.vip_bm_increase);
	}
	if (sd->sc.getSCE(SC_APACHE_EXPBOOST_S)) {
		bonus += sd->sc.getSCE(SC_APACHE_EXPBOOST_S)->val1;
		if (battle_config.vip_bm_increase && pc_isvip(sd)) // Increase Battle Manual EXP rate for VIP
			bonus += (sd->sc.getSCE(SC_APACHE_EXPBOOST_S)->val1 / battle_config.vip_bm_increase);
	}
	...
	// APACHE PREMIUM SERVICE
	if (sd->sc.getSCE(SC_APACHE_JEXPBOOST_A))
		bonus += sd->sc.getSCE(SC_APACHE_JEXPBOOST_A)->val1;
	if (sd->sc.getSCE(SC_APACHE_JEXPBOOST_S))
		bonus += sd->sc.getSCE(SC_APACHE_JEXPBOOST_S)->val1;
...
int pc_dead(map_session_data *sd,struct block_list *src)
{
		...
		&& !mapdata->flag[MF_NOEXPPENALTY] && !mapdata_flag_gvg2(mapdata))
	{
		...
		if ((battle_config.death_penalty_maxlv&1 || !pc_is_maxbaselv(sd)) && base_penalty > 0) {
			...
			t_exp a_base = 0;
			a_base = base_penalty;
			if (sd->sc.getSCE(SC_APACHE_LIFEINSURANCE))
				base_penalty -= (t_exp) ( a_base * ( sd->sc.getSCE(SC_APACHE_LIFEINSURANCE)->val1 / 100. ) );
		...
		if ((battle_config.death_penalty_maxlv&2 || !pc_is_maxjoblv(sd)) && job_penalty > 0) {
			...
			a_job = job_penalty;
			if (sd->sc.getSCE(SC_APACHE_LIFEINSURANCE))
				job_penalty -= (t_exp) ( a_job * ( sd->sc.getSCE(SC_APACHE_LIFEINSURANCE)->val1 / 100. ) );
...
void pc_set_costume_view(map_session_data *sd) {
	int i = -1, head_low = 0, head_mid = 0, head_top = 0, robe = 0, weapon = 0, shield = 0;
	...
	if ((i = sd->equip_index[EQI_HAND_L]) != -1 && (id = sd->inventory_data[i]))
		sd->status.costume_shield = id->look;
