...
void clif_get_weapon_view(map_session_data* sd, t_itemid *rhand, t_itemid *lhand)
{
	...
	*lhand = sd->status.shield;
	...
	if (sd->equip_index[EQI_SHADOW_WEAPON] >= 0 &&
		...
		struct item_data* c_id = sd->inventory_data[sd->equip_index[EQI_SHADOW_WEAPON]];
		if (c_id->view_id > 0)
			*rhand = c_id->view_id;
		else
			*rhand = c_id->nameid;
	...
	if (sd->equip_index[EQI_SHADOW_SHIELD] >= 0 &&
		sd->inventory_data[sd->equip_index[EQI_SHADOW_SHIELD]]){
		struct item_data* c_id = sd->inventory_data[sd->equip_index[EQI_SHADOW_SHIELD]];
		if (c_id->view_id > 0)
			*lhand = c_id->view_id;
		else
			*lhand = c_id->nameid;
...
void clif_map_property(struct block_list *bl, enum map_property property, enum send_target t)
{
	...
	WBUFL(buf,4) = ((mapdata->flag[MF_PVP] || (sd && sd->duel_group > 0))<<0)| // PARTY - Show attack cursor on non-party members (PvP)	WBUFL(buf,4) = ((map_getcell(bl->m, bl->x, bl->y, CELL_CHKPVP) || mapdata->flag[MF_PVP] || (bl->type == BL_PC && ((TBL_PC *)bl)->duel_group > 0))<<0)| // PARTY - Show attack cursor on non-party members (PvP)
	...
	switch( bl->type ){
		...
		case BL_MOB: {
			...
			if( md->guardian_data && md->guardian_data->guild_id ){
				...
				clif_send(&packet, sizeof(packet), src, target);
			} else if (battle_config.show_mob_info && !md->chat_clone_id) { // Chat Clone
				PACKET_ZC_ACK_REQNAMEALL packet = { 0 };
...
void clif_parse_MoveToKafra(int fd, map_session_data *sd)
{
	...
	if (sd->state.storage_flag == 2)
		...
		storage_storageadd(sd, &sd->CollectionStorage, item_index, item_amount = 1);
}
...
void clif_parse_MoveToKafraFromCart(int fd, map_session_data *sd){
	...
	if (sd->state.storage_flag == 1)
		...
		storage_storageaddfromcart(sd, &sd->CollectionStorage, idx, amount = 1);
}
...
void clif_parse_CloseKafra(int fd, map_session_data *sd)
{
	...
	else if (sd->state.storage_flag == 3 || sd->state.storage_flag == 4)
...
void clif_parse_roulette_open( int fd, map_session_data* sd ){
    ...
    if (sd) {
        npc_event(sd, "collection_script::OnOpen", 0);
    }
...
#if PACKETVER >= 20190724
void clif_sale_search_reply( map_session_data* sd, std::shared_ptr<item_data> item ){
#else
	...
	if( info == nullptr ){
		...
		for( uint16 i = REFINE_COST_NORMAL; i < REFINE_COST_MAX; i++ ){
			...
			if( cost != nullptr ){
				...
				if( sd->sc.getSCE(SC_FAILSTACK) ){
					Bonus1 = sd->sc.getSCE(SC_FAILSTACK)->val1;
				}
				
				if( sd->sc.getSCE(SC_REFINEPLUS) ){
					Bonus2 = sd->sc.getSCE(SC_REFINEPLUS)->val1;		
				}
				
				if( sd->sc.getSCE(SC_REFINEPLUS2) ){
					Bonus4 = sd->sc.getSCE(SC_REFINEPLUS2)->val1;		
				}
				/*
...
void clif_parse_refineui_refine( int fd, map_session_data* sd ){
	...
	int Finalcostzeny = cost->zeny;
		Finalcostzeny = (Finalcostzeny * battle_config.refine_eventzeny) /100 ;
	// Try to pay for the refine
	if( pc_payzeny( sd, Finalcostzeny, LOG_TYPE_CONSUME, NULL ) ){
	...
	int Finalrate = cost->chance;	
				if( sd->sc.getSCE(SC_FAILSTACK) ){
					Bonus11 = sd->sc.getSCE(SC_FAILSTACK)->val1*100;
				}
				
				if( sd->sc.getSCE(SC_REFINEPLUS2) ){
					Bonus44 = sd->sc.getSCE(SC_REFINEPLUS2)->val1;		
				}
				
				if( sd->sc.getSCE(SC_REFINEPLUS) ){
					Bonus22 = sd->sc.getSCE(SC_REFINEPLUS)->val1;		
				}
				/*
				if( sd->sc.getSCE(SC_MEMBERLV1) )
					Bonus33 = 1;		
	...
	if( Finalrate >= ( rnd() % 10000 ) ){
		//status_change_end( &sd->bl, SC_FAILSTACK );		 /*extreme_custom_src*/
		...
		/*
		if( sd->sc.getSCE(SC_FAILSTACK) ){
			sc_start(&sd->bl, &sd->bl, SC_FAILSTACK, 100, 1 + sd->sc.getSCE(SC_FAILSTACK)->val1 , 3600000);
			count = sd->sc.getSCE(SC_FAILSTACK)->val1;
			sprintf(atcmd_output, msg_txt(sd,1526), count); // Fail Stack LV. %d
			clif_displaymessage(fd, atcmd_output);
		}else{
			sc_start(&sd->bl, &sd->bl, SC_FAILSTACK, 100, 1, 3600000);
			count = sd->sc.getSCE(SC_FAILSTACK)->val1;
			sprintf(atcmd_output, msg_txt(sd,1526), count); // Fail Stack LV. %d
			clif_displaymessage(fd, atcmd_output);
		}
