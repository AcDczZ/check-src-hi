...
ACMD_FUNC(reload) {
	...
	if (strstr(command, "itemdb") || strncmp(message, "itemdb", 4) == 0) {
		...
		||  prev_config.sell_piece_cap         != battle_config.sell_piece_cap
		||  prev_config.sell_piece_cap_card    != battle_config.sell_piece_cap_card
		)
...
ACMD_FUNC(mobinfo)
{
	...
	for (uint16 k = 0; k < count; k++) {
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			int droprate = mob_getdroprate( &sd->bl, mob, mob->dropitem[i].rate, drop_modifier, nullptr, id->type );

			sprintf(atcmd_output2, " - %s  %02.02f%%\n", item_db.create_item_link( id ).c_str(), (float)droprate / 100);
		...
		if( mob->get_bosstype() == BOSSTYPE_MVP ){
			...
			for (i = 0; i < MAX_MVP_DROP_TOTAL; i++) {
				...
				if (mvppercent > 0) {
					j++;
					if (j == 1) {
						sprintf(atcmd_output2, " - %s  %02.02f%%\n", item_db.create_item_link( id ).c_str(), mvppercent);
					} else {
						sprintf(atcmd_output2, " - %s  %02.02f%%\n", item_db.create_item_link( id ).c_str(), mvppercent);
					}
					strcat(atcmd_output, atcmd_output2);
...
ACMD_FUNC(chatclone) {
	...
	if (sd->chat_clone_delay + clone_delay > (int)time(NULL)) {
		safesnprintf(atcmd_output, CHAT_SIZE_MAX, msg_txt(sd, 1701), sd->chat_clone_delay + clone_delay - (int)time(NULL));
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
...

	{
		...
		if (message[0] != '\"') {
			clif_displaymessage(fd, msg_txt(sd, 1707));
			clif_displaymessage(fd, msg_txt(sd, 1706));
			return -1;
		}
		...
		if (message[i] != '\"') {
			clif_displaymessage(fd, msg_txt(sd, 1708));
			clif_displaymessage(fd, msg_txt(sd, 1706));
			return -1;
		}
		...
		if (message[i] != ' ') {
			clif_displaymessage(fd, msg_txt(sd, 1709));
			clif_displaymessage(fd, msg_txt(sd, 1706));
			return -1;
		}
	...
	if (strlen(title) >= CHATROOM_TITLE_SIZE) {
		safesnprintf(atcmd_output, CHAT_SIZE_MAX, msg_txt(sd, 1713), CHATROOM_TITLE_SIZE);
		clif_displaymessage(fd, atcmd_output);
		return -1;
	}
	...
	if (battle_config.chat_clone_zenycost) {
		if(sd->status.zeny < battle_config.chat_clone_zenycost) {
			safesnprintf(atcmd_output, CHAT_SIZE_MAX, msg_txt(sd, 1715), battle_config.chat_clone_zenycost);
			clif_displaymessage(fd, atcmd_output);
			return -1;
		}
	}
