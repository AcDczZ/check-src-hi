 ...
 */
int mob_getdroprate(struct block_list *src, std::shared_ptr<s_mob_db> mob, int base_rate, int drop_modifier, mob_data* md, enum item_types type)
{
	...
	if (src) {
		...
		if (src->type == BL_PC) { // Player specific drop rate adjustments
			...
			if (sd->sc.getSCE(SC_ITEMBOOST_HE) && mob->status.class_ != CLASS_BOSS)
				drop_rate_bonus += sd->sc.getSCE(SC_ITEMBOOST_HE)->val1;
			...
			if( sd->sc.getSCE(SC_AUTOATTACK) ){
				drop_rate = (int)( 0.5 + ((drop_rate * drop_rate_bonus / 100.)*10/100) );
...
int mob_dead(struct mob_data *md, struct block_list *src, int type)
{
	...
	/*
	if (battle_config.announce_when_mvp_spawn_dead) {
		if (md->get_bosstype() == BOSSTYPE_MVP && map_getmapflag(m, MF_MVP)) {
			if(mvp_sd == nullptr){
			}else{
			char message[128];
			sprintf (message, msg_txt(sd, 1524), mvp_sd->status.name, md->name, mapindex_id2name(mvp_sd->mapindex));
			intif_broadcast2(message, strlen(message) + 1, battle_config.set_announce_color, 0x190, 12, 0, 0);
			}
		} 
	}
	*/
	...
	{ // Item Drop
		...
		for (i = 0; i < MAX_MOB_DROP_TOTAL; i++) {
			...
			drop_rate = mob_getdroprate(src, md->db, md->db->dropitem[i].rate, drop_modifier, md, it->type);
...
// Chat Clone
int mob_clone_spawn_chat(map_session_data *sd, int16 m, int16 x, int16 y, char chat_title[], char chat_msg[]) {
